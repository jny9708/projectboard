...
public class Pagination {
	...
	private int perPageBoardNum = 10;                // 한 페이지당 보여질 게시물의 개수 - listsize 
	private int rangeSize = 5;            // 한 페이지 범위에 보여질 페이지의 개수
	private int page;					// 현재 목록의 페이지 번호
	private int range;					// 각 페이지 범위 시작 번호(몇 번째 블럭인가)
	private int totalBoardCnt;			// 전체 게시물 개수 - listCnt
	private int totalpageCnt;			// 전체 페이지수  - pagecnt
	private int startPage;				// 각 페이지 범위 시작 번호
	private int startBoardNum;			// 해당 페이지에 있는 제일 첫번째 게시물 번호 (시작번호) -startList
	private int endPage;				//각 페이지 범위(블럭) 끝 번호
	private boolean prev;				// 이전 페이지 활성화 여부
	private boolean next;				// 다음 페이지 활성화 여부

	public void pageInfo(int page, int range, int totalBoardCnt) {
